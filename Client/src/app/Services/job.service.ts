...
@Injectable({
  ...
  export class JobService {
    ...
    constructor(private http:HttpClient) { 
      ...
      getJobsFromServer(){
        this.http.get('https://localhost:5002/api/Jobs/GetAllJobs').subscribe((res: any) =>{res.forEach((e:any) => { this.jobs.push(e)});
         })}
...
getUniqueAreasFromServer() {
  this.http.get('https://localhost:5002/api/Jobs/unique-areas').subscribe((res: any) =>{res.forEach((e:any) => { this.uniqueAreas.push(e)});
  });
}
getUniqueFieldsFromServer() {
  this.http.get('https://localhost:5002/api/Jobs/unique-fields').subscribe((res: any) =>{res.forEach((e:any) => { this.uniqueFields.push(e)});
